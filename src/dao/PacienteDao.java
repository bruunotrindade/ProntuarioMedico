...
public class PacienteDao {
	...
	public boolean insert(Paciente p) {
		if(p != null) {
			return mysql.query("INSERT INTO pacientes VALUES (" + p.getNome() + ",'"+p.getCpf()+"','"+p.getDt_nasc()+
	...
	public ArrayList<Paciente> getAll(){
		...
		if(mysql.query("SELECT * FROM pacientes")) {
			ArrayList<HashMap<String, Object>> resultado = mysql.fetch_assoc_all();
			
			for(HashMap<String,Object> r : resultado) {
				Paciente paciente = new Paciente((int) r.get("ID"), (String) r.get("NOME"), (String) r.get("CPF"), (String) r.get("DT_NASCIMENTO"),
	...
	public ArrayList<Paciente> getBy(String filtro, String valor){
		ArrayList<Paciente> pacientes = new ArrayList<Paciente>();
